...
class DiscoursePlugin(Plugin):
	...
	def createForumPost(self, event):
		...
		description = "**Starting:** " + event['startTime'].toString(dateTimeFormat)
		description += '\n' + "**Ending:** " + event['stopTime'].toString(dateTimeFormat)
		description += '\n' + "**Instructor:** " + ' '.join(event['instructorDescription'].split(' ')[1:])
		
		if event['registrationURL'] is not None and event['registrationURL'] != '':
			description += '\n' + "**Register:** " + '[' + event['registrationURL'] + '](' + event['registrationURL'] + ')'

		description += '\n' + "**Price:** " + event['priceDescription'][28:]

		if event['ageDescription']:
			description += '\n' + "**Ages:** " + ' '.join(event['ageDescription'].split(' ')[1:])

		if event['authorizationDescription']:
			description += '\n' + "**Required Authorizations:** " + ', '.join(event['authorizationDescription'].split(' ')[2:])
		...
		title = 'Event notice: ' + event['title'] + ' (' + event['startTime'].toString(dateTimeFormat) + ')'
		discourse_api.create_post(content=description, category_id=int(self.getSetting('Category ID')), topic_id=None, title=title)
		return 
